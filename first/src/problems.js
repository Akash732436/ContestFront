...
const Prob = () => {
  ...
  const Questions = () => {
    let n1 = " ";
    useEffect(() => {
      fetch("http://localhost:3000/api/1/problems")
        .then((res) => {
          return res.json();
        })
        .then((data) => {
          ...
          data.forEach((element) => {
            ...
            s += `<td ><a href="/${id}/${element["question_id"]}/showQuestion">${element["question_name"]} </a></td><td>${element["points"]}</td><td>${element["level"]}</td><td>Users</td></tr>`;
          });
          //s+='</tr>';
          //console.log(s);
